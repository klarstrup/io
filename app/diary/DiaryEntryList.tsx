...
export function DiaryEntryList({
  ...
  return diaryEntries.map(async ([date, { food, workouts }]) => {
            ...
            <div>
              ...
              <ol>
                {(
                  ...
                  .map(([id, workout]) => {
                    ...
                    if (
                      ...
                      if (
                        workingSets.every(
                          ({ inputs }) =>
                            inputs.find(({ type }) => type === InputType.Reps)!
                              .value < 5
                        )
                      ) {
                        successful = false;
                    ...
                    const nextWorkingSet = successful
                      ? ([1, 183, 532].includes(exercise.exercise_id)
                          ? 1.25
                          : 2.5) +
                        (heaviestSet?.inputs.find(
                          (input) => input.type === InputType.Weight
                  ...
                  .sort(
                    (a, b) =>
                      a.workout_timestamp.getTime() -
                      b.workout_timestamp.getTime()
                  )
                  .map(
                    ({
                      exercise_id,
