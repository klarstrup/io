...
export async function getIoClimbAlongCompetitionEvent(
  ...
  const [athletes, circuits, lanes, rounds] = await Promise.all([
    ...
    ClimbAlongRounds.find({ competitionId }).toArray(),
  ]);
  ...
  const ioCircuitIds = ioPerformances
    ...
    : [];
  const [ioCircuitChallengeNodes, holds] = await Promise.all([
    ClimbAlongNodes.find({
  ...
  return {
    ...
    venue: competition.facility,
    eventName: competition.title,
    location: competition.address,
    rounds: await Promise.all(
      ioCircuitChallengeNodes.map(async (ioCircuitChallengeNode) => {
        ...
        const noProblems = problems.filter(
          (problem) =>
            ioCircuitChallengeNode.circuit.circuitId === problem.circuitId,
        ).length;
        ...
        const maxRank = circuitChallengeAthletes?.reduce(
          (max, athlete) =>
        ...
        const noParticipants = circuitChallengeAthletes?.length || NaN;
        ...
        if (
          ...
          const zoneScore =
            descendingHolds[descendingHolds.length - 1]?.holdScore;
          ...
          scores.push({
            ...
            percentile: percentile(
              ioPerformanceSum.rank,
              maxRank ?? noParticipants,
            ),
            tops: topSums.reduce(
        ...
        const lane = lanes.find(({ startNodeId, startEdgeId }) =>
          ioCircuitChallengeNode.inputsFrom.some(
            ({ nodeId, edgeId }) =>
              nodeId === startNodeId && edgeId === startEdgeId,
          ),
        );
        const round =
          lane && rounds.find((round) => round.roundId === lane.roundId);
